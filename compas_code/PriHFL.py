...
class Client:
    def __init__(self, client_id, data, sensitive_features, batch_size, learning_rate, model_class, input_size, attack_form=None):
        ...
        self.attack_form = attack_form  # "no_attack", "attack7", "attack2"
...
class Server:
    def __init__(self, global_model, clients, algorithm, hyperparams, server_data=None):
        """
        初始化服务器类（适用于 PriHFL 和 PriHFL_RW）
        """
    ...
    def aggregate(self, client_updates=None):
        """
        聚合客户端模型更新并更新全局模型权重（基于简单平均）
        """
        if self.algorithm in ['FedAvg', 'FedAvg_RW', 'PriHFL', 'PriHFL_RW'] and client_updates is not None:
    ...
    def _average_weights(self, client_updates):
        ...
        for key in client_updates[next(iter(client_updates))].keys():
            aggregated_weights[key] = torch.stack(
                [client_updates[client_id][key].float().to(DEVICE) for client_id in client_updates],
                dim=0
            ).mean(dim=0)
    ...
    def run_round(self, global_model, test_df, y_test_values, model_class):
        ...
        if self.server_data is not None:
            ...
            for epoch in range(HYPERPARAMETERS['SERVER_EPOCHS_PriHFL']):
        ...
        # 修改为接收 test_inference_modified 返回7个值
