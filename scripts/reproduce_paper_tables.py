...
def load_bundle(dataset: str, alpha: float, config: ExperimentConfig, device: torch.device) -> Dict[str, Any]:
    ...
    label_col = "two_year_recid" if dataset == "compas" else "income"
    ...
    if dataset == "acs_income":
        if config.include_sensitive_feature:
            raise ValueError("ACS SEX protocol excludes the sensitive feature")
        root_indices = set(server_df.index)
        remaining_indices = set(loader.train_df.index) - root_indices
        loader.preprocessing_audit["partition"] = {
            "root_is_training_subset": root_indices <= set(loader.train_df.index),
            "root_client_overlap": len(root_indices & remaining_indices),
            "root_rows": len(root_indices), "client_pool_rows": len(remaining_indices),
            "root_original_row_ids_sha256": hashlib.sha256(loader.train_original_row_ids[server_df.index].astype("<i8").tobytes()).hexdigest(),
        }
        assert root_indices <= set(loader.train_df.index) and not root_indices & remaining_indices
