#!/usr/bin/env python3
"""Final candidate for readable server/root distribution sensitivity.

The main table uses only the stable operating region:
1. IID server/root: stratified clean 10% server/root data.
2. Mild non-IID server/root: controlled majority sensitive-label skew 0.05.
3. Moderate non-IID server/root: controlled majority sensitive-label skew 0.10.

Extreme skew rows are intentionally excluded from the paper-facing table because
they can create degenerate evaluation behavior where fairness metrics look low
for the wrong reason. Raw runs remain auditable in the JSONL.
"""
...
OUT_DIR = ROOT / "results" / "paper_tables" / "goal_revision_v6"
RUNNER = ROOT / "scripts" / "reproduce_paper_tables.py"
PY = Path(sys.executable)
SUITE = "goal_server_iid_sensitivity_v6"

LEVELS: List[Tuple[str, str, Optional[float], int]] = [
    ("IID server/root", "stratified_sensitive", None, 0),
    ("Mild non-IID server/root", "controlled_group_skew", 0.05, 1),
    ("Moderate non-IID server/root", "controlled_group_skew", 0.10, 2),
]
...
def run_grid(rounds: int, seeds: Sequence[int], datasets: Sequence[str], dry_run: bool) -> None:
    for seed in seeds:
        for level_name, sampling, skew, _ in LEVELS:
...
def correlations(agg_rows: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for key, vals in sorted(groups.items()):
        ...
        out.append({
            ...
            "delta_acc_moderate_minus_iid": vals[-1]["ACC_pct_mean"] - vals[0]["ACC_pct_mean"] if len(vals) >= 2 else float("nan"),
            "delta_fairavg_moderate_minus_iid": vals[-1]["FairAvg_mean"] - vals[0]["FairAvg_mean"] if len(vals) >= 2 else float("nan"),
        })
    return out
...
def main() -> None:
    ...
    if args.summarize:
        ...
        write_csv(OUT_DIR / "goal_server_iid_sensitivity_v6_raw.csv", rows)
        write_csv(OUT_DIR / "goal_server_iid_sensitivity_v6_by_level.csv", agg)
        write_csv(OUT_DIR / "goal_server_iid_sensitivity_v6_correlations.csv", correlations(agg))
