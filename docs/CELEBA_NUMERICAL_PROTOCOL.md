...
Strict CUDA coordinate median rejected its unused nondeterministic tied-index output during the first attempt. Only strict-mode CUDA median values now execute the same torch.median on CPU and return to the original device; lower-median, ties and NaN propagation are exactly tested against the original CUDA values. Other aggregation operations and non-strict/tabular paths are unchanged. Failed diagnostic attempts are retained.
