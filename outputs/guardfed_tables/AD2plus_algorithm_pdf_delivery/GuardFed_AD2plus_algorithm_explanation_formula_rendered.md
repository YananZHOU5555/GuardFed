...
- 性能攻击：恶意客户端上传反向、放大或离群更新，使全局模型 accuracy 下降。
- 公平性攻击：恶意客户端不一定明显降低 accuracy，但会扩大不同敏感群体之间的错误率或 positive prediction rate 差异，使 AEOD/ASPD 变差。
...
例子：如果客户端 A 的更新在 root data 上得到 84.0% accuracy，而客户端 B 得到 81.0%，在其它风险相近时，A 的 utility 项更高，AD2+ 会给 A 更高权重。

## 4. Fairness risk：同时检查 AEOD 和 ASPD
...
例子：一个更新让 accuracy 提升到 84%，但 AEOD=0.15、ASPD=0.12；另一个更新 accuracy 为 83.5%，但 AEOD=0.02、ASPD=0.03。FedAvg 可能更偏向第一个更新，AD2+ 会识别第一个更新的公平风险，并降低它的聚合权重。

## 5. Fairness budget 与动态 violation
...
例子：若 $B=0.12$。客户端 A 的 $F_i^t=0.08$，则 $V_i^t=0$；客户端 B 的 $F_i^t=0.18$，则 $V_i^t=0.06$。如果这一轮很多客户端都超过预算，$\eta^t$ 也会增大，B 类更新会被更强地降权。

## 6. Robust centrality：识别离群更新
...
例子：20 个客户端中有 16 个更新方向相近，而 4 个恶意客户端上传反向或极端更新。这 4 个更新通常距离 median center 更远，centrality 更低，因此 AD2+ 会降低它们的权重。

## 7. Root alignment：检查是否朝可信方向优化
...
例子：性能攻击可能让恶意客户端更新偏离正常优化方向。即使该更新在某些局部指标上看起来不差，只要它和 clean root update 方向明显不一致，alignment 项就会降低其总分。

## 8. Robust normalization：避免不同指标量纲不一致
...
其中 $c$ 是 score clipping 上限，MAD 是 median absolute deviation。

对于越大越好的信号，例如 utility、centrality、alignment，直接使用 $Z(\cdot)$。对于越小越好的信号，例如 fairness risk 和 violation，先把它们写成 penalty，再从 reward 中减掉。

## 9. AD2+ 核心得分：reward minus penalty
...
- accuracy 提升越明显，得分越高；
- 越接近多数正常客户端，得分越高；
- 越接近 clean root update 方向，得分越高；
- AEOD/ASPD 越差，得分越低；
- 超过公平预算越多，得分越低；
- 当本轮整体公平风险升高时，violation 惩罚自动增强。
...
例子：某客户端只是轻微可疑，但仍包含有用训练信息。硬过滤会直接丢弃它；AD2+ 可以降低它的权重，但不完全浪费它携带的有效信息。

## 12. Root-norm scaling：限制异常更新幅度
...
例子：如果恶意客户端让聚合更新范数远大于 clean root update，root-norm scaling 会把整体更新拉回可信尺度，降低模型漂移风险。

## 13. 为什么它可以称为 adaptive

AD2+ 的 adaptive 体现在每一轮都会根据当前训练状态重新计算聚合行为：

- 每轮重新评估每个客户端更新在 root data 上的 clean utility；
- 每轮重新计算 AEOD/ASPD 和 fairness violation；
- 每轮重新计算客户端更新的 centrality 和 root alignment；
- 每轮根据平均 fairness risk 更新 dual multiplier $\eta^t$；
- 每轮用新的 score 生成新的 softmax 聚合权重；
- 每轮根据 clean root update 重新缩放更新范数。

因此，即使 $B,\alpha,\beta,\gamma,\lambda_f,\lambda_v$ 是预设超参数，客户端权重和最终全局更新仍会随攻击强度、数据分布、训练阶段和公平风险动态变化。

## 14. 算法流程

1. 输入全局模型 $w^t$、客户端更新 $\{\Delta_i^t\}_{i=1}^m$、clean root data $D_r$、clean root update $\Delta_r^t$。
2. 对每个客户端构造候选模型 $w_i^t=w^t+\Delta_i^t$。
3. 在 $D_r$ 上计算 clean utility $U_i^t$。
4. 在 $D_r$ 上计算 AEOD、ASPD，并得到 fairness risk $F_i^t$。
5. 计算 fairness violation：$V_i^t=\max(0,F_i^t-B)$。
6. 计算本轮动态 multiplier：$\eta^t=\operatorname{softplus}((\bar F^t-B)/\tau)$。
7. 计算 robust centrality $C_i^t$。
8. 计算 root alignment $A_i^t$。
9. 对各项信号做 robust normalization。
10. 计算 $\mathcal{R}_i^t$、$\mathcal{P}_i^t$ 和 $s_i^t=\mathcal{R}_i^t-\mathcal{P}_i^t$。
11. 通过 softmax 把 $s_i^t$ 转换为聚合权重 $p_i^t$。
12. 进行加权聚合并执行 root-norm scaling。
13. 输出下一轮全局模型 $w^{t+1}$。

## 15. 可以放进论文 method 的中文表述
...
- Clean server/root data 用于服务器端评估和校准，不要求客户端上传额外敏感信息。
- AD2+ 不是简单组合 FedAvg、FairFed、FLTrust 或 FairGuard，也不是从这些方法里选择最优结果；它是客户端更新层面的独立自适应聚合规则。
- 固定公平预算 $B$ 的作用类似约束阈值；动态性主要来自每轮重新计算的 fairness risk、violation、dual multiplier 和聚合权重。
- 如果要写得更理论化，可以把 AD2+ 描述为 fairness-constrained robust aggregation 的 primal-dual surrogate。
