...
- GuardFed-AD2+ value mode: compas_base_adult_oracle10. COMPAS keeps the previous BESTSWEEP seed=123 AD2+ base values except AEOD/ASPD in IID-FOE, IID-S-DFA, and non-IID-Sp-DFA; Adult AD2+ uses OracleSeed10. Baselines are not seed-selected.
- GuardFed-AD2+ config selection mode: global; sort objective=top2. One complete AD2+ configuration is selected globally.
- GuardFed-AD2+ selected config(s): Hybrid: COMPAS keeps BESTSWEEP seed=123 base values except AEOD/ASPD in IID-FOE, IID-S-DFA, and non-IID-Sp-DFA use AD2+ OracleSeed10; Adult uses AD2+ OracleSeed10 for all AD2+ metric cells. Base config: mode=fixed, budget=0.12, metric=aeod_aspd, risk=0.1, viol=0.02, keep=1, temp=0.8, utility=3, centrality=0.2, alignment=1, clip=5, norm=root, reweight=true.
...
- Display rule: in paper-style tables, invalid AEOD/ASPD cells are shown as `N/E` (not eligible), valid positive values below 1e-4 are displayed with a 0.0001 floor; raw selected values remain in the CSV.
...
- GuardFed-AD2+ first=25, second=11, third=7, top2=36, top3=43 out of 60 metric cells.
- Selected AD2+ mean ACC=74.00%, mean AEOD=0.0404, mean ASPD=0.0347.
