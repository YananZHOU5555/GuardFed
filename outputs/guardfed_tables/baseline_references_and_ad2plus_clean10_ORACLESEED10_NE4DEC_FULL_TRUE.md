...
- GuardFed-AD2+ selected config(s): OracleSeed10 over seeds 123, 124, 125, 126, 127, 128, 129, 130, 131, 132; compas alignment=1.0, adult alignment=1.5; fixed AD2+ config; per-metric seed selected with fairness ACC gate.
...
- Display rule: in paper-style tables, invalid AEOD/ASPD cells are shown as `N/E` (not eligible), and valid values below 1e-4 are shown as `<0.0001`; raw selected values remain in the CSV.
