...
import argparse, copy, csv, json, math, random, subprocess, sys, time
...
def compute_metrics(y_true: np.ndarray, y_pred: np.ndarray, sensitive: np.ndarray) -> Dict[str, Any]:
    ...
    return {"accuracy": accuracy, "aeod": float(aeod) if not math.isnan(aeod) else math.nan, "aspd": float(aspd) if not math.isnan(aspd) else math.nan, "warnings": warnings}
...
def evaluate_for_reporting(method: str, model: nn.Module, bundle: Dict[str, Any], config: ExperimentConfig) -> Dict[str, Any]:
    if method in {"GuardFed-AD2", "GuardFed-AD2+", "LoGoFair"}:
...
def sample_server_dataframe(
    ...
    audit = {
        ...
        "server_group_counts": {f"{int(k[0])}|{int(k[1])}": int(v) for k, v in server_df.groupby([sensitive_col, label_col]).size().sort_index().to_dict().items()},
    }
    return server_df, audit


def load_bundle(dataset: str, alpha: float, config: ExperimentConfig, device: torch.device) -> Dict[str, Any]:
    set_seed(config.seed); loader = DatasetLoader(dataset_name=dataset, seed=config.seed, device=str(device)); label_col = "income" if dataset == "adult" else "two_year_recid"
...
def main() -> int:
    ...
    p.add_argument("--server-sampling", default="stratified_sensitive", choices=["stratified_sensitive", "iid_random", "dirichlet_strata", "dirichlet_strata_floor", "dirichlet_label_preserved", "dirichlet_label_preserved_strong_floor", "group_balanced", "sensitive_balanced", "class_balanced"])
